...
class LiveSourceListView(SourceQuerysetMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        def get_dynamic_url(url_str, default_prefix=''):
            if not url_str:
                return url_str
            ...
            if use_proxy_path:
                ...
                if ':8889' in url_str or '/webrtc' in url_str:
                    prefix = '/webrtc'
                elif ':8888' in url_str or '/hls' in url_str:
                    prefix = '/hls'
        ...
        if db_settings:
            context['mediamtx_webrtc_url'] = get_dynamic_url(db_settings.mediamtx_webrtc_url, default_prefix='/webrtc')
            context['mediamtx_hls_url'] = get_dynamic_url(db_settings.mediamtx_hls_url, default_prefix='/hls')
            context['stun_url'] = db_settings.stun_url
            context['turn_url'] = db_settings.turn_url
        else:
            context['mediamtx_webrtc_url'] = get_dynamic_url(getattr(settings, 'MEDIAMTX_WEBRTC_URL', 'http://127.0.0.1:8889'), default_prefix='/webrtc')
            context['mediamtx_hls_url'] = get_dynamic_url(getattr(settings, 'MEDIAMTX_HLS_URL', 'http://127.0.0.1:8888'), default_prefix='/hls')
